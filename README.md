...
| background / b | \_                                                              | `/r_45,b_00ff00/buffalo.png`                                     | Background colour used by `extend`, `rotate`, `flatten`, `round`, `opacity` and `resize` (with `fit_contain`).                                                                                                                                     |
...
| autoOrient / autoorient | [Docs](https://sharp.pixelplumbing.com/api-operation#autoorient) | `/autoOrient/buffalo.png`                              | Rotate and flip the image based on its EXIF `Orientation` tag, then remove the tag.                                                                                                                                                               |
...
| dilate         | [Docs](https://sharp.pixelplumbing.com/api-operation#dilate)    | `/dilate_3/buffalo.png`                                          | Expand foreground objects. Width in pixels, an integer between `1` and `1000` (defaults to `1`).                                                                                                                                                   |
| erode          | [Docs](https://sharp.pixelplumbing.com/api-operation#erode)     | `/erode_3/buffalo.png`                                           | Shrink foreground objects. Width in pixels, an integer between `1` and `1000` (defaults to `1`).                                                                                                                                                   |
| clahe          | [Docs](https://sharp.pixelplumbing.com/api-operation#clahe)     | `/clahe_50_50_3/buffalo.png`                                     | Contrast limiting adaptive histogram equalization, as `{width}_{height}_{maxSlope}`. `width` is required and `height` defaults to it (a square window); both are integers between `1` and `65536`. `maxSlope` is an integer between `0` and `100` (defaults to `3`). |
...
| normalize / normalise | [Docs](https://sharp.pixelplumbing.com/api-operation#normalize) | `/normalize_1_99/buffalo.png`                             | Stretch the luminance to the full dynamic range, as `{lower}_{upper}` percentiles. `lower` is a number between `0` and `99` (defaults to `1`), `upper` between `1` and `100` (defaults to `99`) and has to be greater than `lower`.                |
| threshold      | [Docs](https://sharp.pixelplumbing.com/api-operation#threshold) | `/threshold_10/buffalo.png`                                      | `{threshold}_{greyscale}`. `threshold` is an integer between `0` and `255` (defaults to `128`). Optional `greyscale` (`true` by default) converts to single channel greyscale first, e.g. `/threshold_128_false/`.                                  |
| linear         | [Docs](https://sharp.pixelplumbing.com/api-operation#linear)    | `/linear_1.2_-10/buffalo.png`                                    | Levels adjustment applying `a * input + b`, as `{a}_{b}`. `a` is a number between `-1000` and `1000` (defaults to `1`), `b` between `-255` and `255` (defaults to `0`).                                                                            |
| tint           | [Docs](https://sharp.pixelplumbing.com/api-colour#tint)         | `/tint_00ff00/buffalo.png`                                       | Tint colour.                                                                                                                                                                                                                                      |
| grayscale / greyscale | [Docs](https://sharp.pixelplumbing.com/api-colour#grayscale) | `/grayscale/buffalo.png`                                     |                                                                                                                                                                                                                                                   |
...
| brightness     | [Docs](https://sharp.pixelplumbing.com/api-operation#modulate)  | `/brightness_1.25/buffalo.png`                                   | Brightness multiplier, a number between `0` and `100`. Required.                                                                                                                                                                                  |
| saturation     | [Docs](https://sharp.pixelplumbing.com/api-operation#modulate)  | `/saturation_0.5/buffalo.png`                                    | Saturation multiplier, a number between `0` and `100`. Required.                                                                                                                                                                                  |
...
| round          | \_                                                              | `/round_20/buffalo.png`                                          | Round the corners with the given radius in pixels, a number between `0` and `10000`. Without a radius (`/round/`) the image is made as round as it can be: a circle when square, a pill otherwise. Corners are cut out, so the output format needs an alpha channel (`png`, `webp`, `avif`) unless the `background` / `b` colour is set to fill them, e.g. `/round,b_fff,f_jpeg/`. Applied after every other modifier. |
