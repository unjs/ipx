...
| round          | \_                                                              | `/round_20/buffalo.png`                                          | Round the corners with the given radius in pixels, a number between `0` and `10000`. Without a radius (`/round/`) the image is made as round as it can be: a circle when square, a pill otherwise. Corners are cut out, so the output format needs an alpha channel (`png`, `webp`, `avif`) unless the `background` / `b` colour is set to fill them, e.g. `/round,b_fff,f_jpeg/`. Applied after every other modifier, which means the whole output has to be held as raw pixels at once: a request whose output would exceed the `maxOutputDimension` budget (an animated image stacks every frame, so it gets there first) is rejected with a `400 IPX_OUTPUT_TOO_LARGE`. |
