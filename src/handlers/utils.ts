...
import type { Handler, Context, ImageMeta } from "../types";
...
export function getHandler (key: string): Handler {
  // @ts-ignore
  // eslint-disable-next-line import/namespace
  return Handlers[key];
}

export function applyHandler (context: Context, pipe: Sharp, handler: Handler, argumentsString: string) {
  const arguments_ = handler.args ? parseArgs(argumentsString, handler.args) : [];
  return handler.apply(context, pipe, ...arguments_);
}

export function clampDimensionsPreservingAspectRatio (sourceDimensions: ImageMeta, desiredDimensions: {width: number, height: number}) {
